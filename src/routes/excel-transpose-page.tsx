...
function PreviewTable({
  ...
  if (loading) {
    return (
      <div className="flex h-72 items-center justify-center gap-2 text-sm text-foreground-muted">
        <Loader2 className="h-4 w-4 animate-spin" />
        正在计算…
      </div>
    );
  ...
  if (!preview) {
    return (
      <div className="flex h-72 flex-col items-center justify-center gap-3 text-foreground-muted">
  ...
  if (preview.rows.length === 0) {
    return (
      <div className="flex h-72 flex-col items-center justify-center gap-2 text-foreground-muted">
  ...
  return (
    <ScrollArea className="h-[min(560px,calc(100vh-360px))]">
