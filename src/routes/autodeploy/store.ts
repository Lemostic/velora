...
const FALLBACK_NODE_TYPES: NodeType[] = [
  ...
  },
  {
    ...
    fields: [
      { name: "source_path", label: "路径（可选）", kind: "text", required: false, placeholder: "可由上游远端节点提供" },
      { name: "mode", label: "权限模式", kind: "text", required: true, placeholder: "755" },
      { name: "recursive", label: "递归处理", kind: "checkbox", required: false, default: "false" },
    ],
  },
  {
    id: "remote_chown",
    ...
    fields: [
      { name: "source_path", label: "路径（可选）", kind: "text", required: false, placeholder: "可由上游远端节点提供" },
      { name: "owner", label: "用户（可选）", kind: "text", required: false, placeholder: "deploy" },
      { name: "group", label: "组（可选）", kind: "text", required: false, placeholder: "www-data" },
      { name: "recursive", label: "递归处理", kind: "checkbox", required: false, default: "false" },
    ],
  },
  {
  ...
  },
  {
    ...
    outputs: 0,
    fields: [
      { name: "title", label: "通知标题", kind: "text", required: true, placeholder: "部署完成" },
      { name: "body", label: "通知内容", kind: "text", required: true, placeholder: "前端 dist 已上传到 /var/www/app" },
      {
        name: "level",
        label: "通知级别",
        kind: "select",
