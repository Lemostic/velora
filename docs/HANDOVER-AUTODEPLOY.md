...
`自动化部署` 是一个 Node-RED 风格的工作流编辑器模块，给 Velora 加了内置节点，让用户能拖拽编排本地处理、SFTP 传输和远端服务器内操作。
...
| 真 SFTP 执行 | ✅ `ssh2`，支持密码 / 私钥认证 |
| SSH 会话复用 | ✅ `ssh_session` 建立一次连接，后续远端节点通过 `sessionId` 复用 |
...
`tar` 或 `unzip`。如果服务器没有对应命令，节点会返回远端命令的退出码和错误信息。
